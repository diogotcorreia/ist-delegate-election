...
function BulkCreateElectionsSubmitButton({ candidacyStart, candidacyEnd, votingStart, votingEnd, round, selectedYears }: BulkCreateElectionsSubmitButtonProps) {
const submit =useSubmit();
    const payload: BulkCreateElectionsDto = useMemo(() => ({
        candidacyPeriod: (candidacyStart !== null && candidacyEnd !== null) ? {start: candidacyStart, end: candidacyEnd} : undefined,
        votingPeriod: {start: votingStart, end: votingEnd},
        round,

        degrees: selectedYears.flatMap((degrees, year) => 
            [...degrees].map((degree) => ({degreeId: degree, curricularYear: year === 0 ? undefined : year}))
        )
  ...
  return (
    <Button onClick={handleClick}>Create {payload.degrees.length} elections</Button>
  );
}
